...
CAV_DATA = ("/Users/alexkolar/Library/CloudStorage/Box-Box/Zhonglab/Lab data/Ring Resonators"
            "/New_mounted_device/10mK/03262025/SDS00003.csv")
FREQ_RANGE = (194807.649, 194816.359)
FREQ_RANGE_CAV = (194805.651, 194814.283)
...
tau_err *= 1e3  # convert to ms


# CAVITY DATA
cavity_df = pd.read_csv(CAV_DATA, header=10, skiprows=[11])

ramp = cavity_df['CH1'].astype(float)
transmission_cav = cavity_df['CH2'].astype(float)

id_min = np.argmin(ramp)
id_max = np.argmax(ramp)
transmission_cav = transmission_cav[id_min:id_max]
transmission_cav.reset_index(drop=True, inplace=True)
freq_cav = np.linspace(FREQ_RANGE_CAV[0], FREQ_RANGE_CAV[1],
                       num=(id_max-id_min))  # unit: GHz
freq_cav += AOM_OFFSET - ref_freq
...
axs[0].plot(freq, avg_trans,
            color=color_spec)
axs[1].plot(freq_cav, transmission_cav,
            color='mediumaquamarine')
ax_r = axs[2].twinx()
axs[2].errorbar(pl_freqs, area, yerr=area_err,
                marker='o', ls='', capsize=3, color=color_pl)
ax_r.errorbar(pl_freqs, tau, yerr=tau_err,
...
axs[2].set_ylabel('PL Area (A.U.)', color=color_pl)
ax_r.set_ylabel('PL Lifetime (ms)', color=color_lifetime)
