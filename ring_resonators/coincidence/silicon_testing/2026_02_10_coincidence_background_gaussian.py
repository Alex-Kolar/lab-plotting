...
OUTPUT_DIR = ('/Users/alexkolar/Desktop/Lab/lab-plotting/output_figs/ring_resonators/silicon_testing'
              '/silicon_mk_5/chip_2/pair_generation/pulse_power_sweep_2026_02_10/gaussian_fit_background')
...
for key, val in POWER_DICT.items():
    ...
    time_to_fit = df['Time(ps)'][idx_to_fit] / 1e6  # convert to us
    counts_to_fit = df['Counts'][idx_to_fit]
    center_guess = coincidence_center_guess + 2
    sigma_guess = 0.015
    amplitude_guess = max(counts_to_fit)*sigma_guess*np.sqrt(2*np.pi)
    pulse_res = bg_model.fit(counts_to_fit, x=time_to_fit,
                             center=center_guess,
                             sigma=sigma_guess,
                             amplitude=amplitude_guess)
    all_res_bg.append(pulse_res)

    plt.plot(time_to_fit, counts_to_fit,
             color='coral', label='Data')
    plt.plot(time_to_fit, pulse_res.best_fit,
    ...
    plt.plot(time_to_fit, counts_to_fit - pulse_res.best_fit,
             color='mediumpurple')

    plt.title(f'Background for Output Power {val} uW (with Background Fit Removed)')
    plt.xlabel(r'Time ($\mathrm{\mu}$s)')
    plt.ylabel('Counts')
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'background_fits', f'background_fit_no_background_{key}mA.png'))
    plt.clf()
    ...
    time_to_fit = df['Time(ps)'][idx_to_fit] / 1e6  # convert to us
    counts_to_fit = df['Counts'][idx_to_fit]
    counts_no_bg = counts_to_fit - pulse_res.best_fit  # subtract background fit
    ...
    plt.plot(time_to_fit, counts_to_fit,
             color='coral', label='Data')
    plt.plot(time_to_fit, pulse_res.best_fit,
             ls='--', color='gray', label='Background Fit')
    plt.plot(time_to_fit, peak_res.best_fit+pulse_res.best_fit,
             ls='--', color='k', label='Fit')
...
# plot of g(2) information
plt.errorbar(list(POWER_DICT.values()), all_peak_height,  # yerr=all_peak_height_err,
             ls='', marker='o', capsize=3, color='cornflowerblue')

plt.xlabel(r'Output Power ($\mathrm{\mu}$W)')
plt.ylabel(r'Peak Height')
plt.xscale('log')
plt.yscale('log')
plt.tight_layout()
plt.show()
