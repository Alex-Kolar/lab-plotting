...
from cav_transmission import R_mod_lmfit_lorentzian, R_mod_lmfit_gaussian
...
if ion_distribution == 'lorentzian':
    model = Model(R_mod_lmfit_lorentzian)
elif ion_distribution == 'gaussian':
    model = Model(R_mod_lmfit_gaussian)
else:
    raise ValueError('Ion distribution not recognized')
params = Parameters()
...
plt.title(f'Cavity Coupling Fit ({ion_distribution})'.title())
...
# print reports
print('Cavity Fit:')
print(res_cavity.fit_report(show_correl=False))
print(f'Ion Fit ({ion_distribution}):')
