...
# compute ifft
od_wrapped = np.roll(od, len(od)//2)
output = ifft(od_wrapped)
freq_spacing = freq[1] - freq[0]
output_time = np.linspace(0, (1/freq_spacing)*len(output), len(output))

plt.plot(output_time, output.real,
         color='coral')
plt.xlim(0, 1)
