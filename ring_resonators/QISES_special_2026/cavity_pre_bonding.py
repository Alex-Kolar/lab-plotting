...
DATA = ("/Users/alexkolar/Library/CloudStorage/Box-Box/Zhonglab/Lab data/Ring Resonators"
        "/mounted_device_mk_2/300K/07192024/SDS00015.csv")
...
freq_to_plot
norm_val = out.params['p0_amplitude'].value + out.params['p1_amplitude'].value + out.params['c'].value
print(norm_val)
ax.plot(freq_to_plot, transmission[idx_to_fit]/norm_val, color=color, label="Transmission")
ax.plot(freq_to_plot, out.best_fit/norm_val, '--k', label="Fit")
ax.set_xlim(-1000, 1000)
# ax.grid(True)
# ax.legend(shadow=True)
wavelength = (3e8 / (freq_light * 1e6)) * 1e9
ax.set_title("High Q Resonance")
ax.set_xlabel("Cavity Detuning (MHz)")
ax.set_ylabel("Cavity Reflection (Normalized)")
