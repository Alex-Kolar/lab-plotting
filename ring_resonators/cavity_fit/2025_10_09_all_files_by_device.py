...
DATA_DIR = ("/Users/alexkolar/Library/CloudStorage/Box-Box/Zhonglab/Lab data/Ring Resonators"
            "/Mounted_device_mk_4/300K/2025_10_09/cavity_scan")
CSV_PATH = ("/Users/alexkolar/Library/CloudStorage/Box-Box/Zhonglab/Lab data/Ring Resonators"
            "/Mounted_device_mk_4/300K/2025_10_09/cavity_scan/resonances_10_09_2025.csv")
...
OUTPUT_DIR = ("/Users/alexkolar/Desktop/Lab/lab-plotting/output_figs/ring_resonators"
              "/mounted_mk_4/300K_cavity_unbonded/300K_10092025")
OUTPUT_DIR_ALL = ("/Users/alexkolar/Desktop/Lab/lab-plotting/output_figs/ring_resonators"
                  "/mounted_mk_4/300K_cavity_unbonded/all_devices")
...
SAVE_DEVICE_PLOTS = True  # if true, save combined output plots. Otherwise, display with plt.show()
PLOT_FIELD_ENHANCE = True  # plot field enhancement factor for each device
PLOT_RATE = True  # plot rate of pair generation for each device
...
for row_idx, row in main_df.iterrows():
    ...
    freq_start = row['Start']  # unit: GHz
    freq_end = row['End']  # unit: GHz
...
labels = sorted(list(q_by_device.keys()))
bar_x = range(len(q_by_device.keys()))
# bar_y = [max(vals) for vals in q_by_device.values()]
bar_y = [max(q_by_device[device]) for device in labels]
plt.bar(bar_x, bar_y,
...
# make bar graph of highest rate
bar_y_under = []
bar_y_over = []
for device in labels:
    centers = np.array(center_by_device[device]) * 1e-6  # convert to THz
    qs = np.array(q_by_device[device])
    contrasts = np.array(contrast_by_device[device])

    if device <= 10:
        n_photons_1, n_photons_2 = calculate_rates(centers, qs, contrasts, power=1, L=(np.pi*440e-6))
    else:
        n_photons_1, n_photons_2 = calculate_rates(centers, qs, contrasts, power=1, L=(np.pi*220e-6))

    max_under = n_photons_1.max()
    max_over = n_photons_2.max()
    bar_y_under.append(max_under)
    bar_y_over.append(max_over)

plt.bar(bar_x, bar_y_over,
        color=color_rate, edgecolor='k', zorder=2)
plt.xticks(bar_x, labels)
plt.title("Overcoupled Pair Rate")
plt.xlabel("Device Number")
plt.ylabel("Highest Pair Rate (pairs/s/mW^2)")
plt.grid(axis='y')

plt.tight_layout()
plt.show()

plt.bar(bar_x, bar_y_under,
        color=color_rate, edgecolor='k', zorder=2)
plt.xticks(bar_x, labels)
plt.title("Undercoupled Pair Rate")
plt.xlabel("Device Number")
plt.ylabel("Highest Pair Rate (pairs/s/mW^2)")
plt.grid(axis='y')
...
for device in center_by_device:
    ...
    if PLOT_FIELD_ENHANCE:
        ...
        if SAVE_DEVICE_PLOTS:
            if not os.path.exists(os.path.join(OUTPUT_DIR_ALL, 'enhancement')):
                os.makedirs(os.path.join(OUTPUT_DIR_ALL, 'enhancement'))
    ...
    if PLOT_RATE:
        ...
        if SAVE_DEVICE_PLOTS:
            if not os.path.exists(os.path.join(OUTPUT_DIR_ALL, 'rates')):
                os.makedirs(os.path.join(OUTPUT_DIR_ALL, 'rates'))
