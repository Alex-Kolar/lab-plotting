...
DATA_DIR = ("/Users/alexkolar/Library/CloudStorage/Box-Box/Zhonglab/Lab data/Ring Resonators"
            "/New_mounted_device/300K/07182024")
CSV_PATH = ("/Users/alexkolar/Library/CloudStorage/Box-Box/Zhonglab/Lab data/Ring Resonators"
            "/New_mounted_device/300K/07182024/resonances_07_18_2024.csv")
...
for _, row in main_df.iterrows():
    ...
    if device in center_by_device:
        ...
        freq_by_device[device].append(freq)
        trans_by_device[device].append(transmission)
    else:
        center_by_device[device] = all_center_curr
        q_by_device[device] = all_q_curr
        freq_by_device[device] = [freq]
        trans_by_device[device] = [transmission]
...
for device in center_by_device:
    ...
    ax.stem(centers, q_by_device[device],
             linefmt=color, markerfmt=color, basefmt='k')
    ax.set_title(f"Resonances")
...
# make plot of resonances versus freq for each device
for device in freq_by_device:
    fig, ax = plt.subplots(figsize=(4, 3), dpi=300)
    # ax.stem(freq_by_device[device], q_by_device[device],
    #         linefmt=color, markerfmt=color, basefmt='k')
    for freq, trans in zip(freq_by_device[device], trans_by_device[device]):
        plt.plot(freq, trans)
    ax.set_title(f"Resonances")
